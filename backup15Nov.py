...
"""
    def response(self):
        return requests.get(("https://openfarm.cc/api/v1/crops/" + self.plant)) 

    def name():
        return response().json()["data"]["attributes"]["name"]

    def binomial_name():
        return response().json()["data"]["attributes"]["binomial_name"]

    def description():
        return response().json()["data"]["attributes"]["description"]

    def short_description():
        return str(description()[ 0 : 250 ]) + "..."

    def common_names():
        return response().json()["data"]["attributes"]["common_names"]

    def image():
        return response().json()["included"][0]["attributes"]["image_url"]
"""
...
info.update("pear")
print(info.description)
# To change plant >>> info.plant = "nameHere"
"""
...
pd = {
        ...
        "ff": info.plant,
        "shortDesc": info.short_description,
        "longDesc" : info.description,
        "temperature" : 27.5,
        "humidity" : 17,
        "LED": True,
        "speaker": False,
...
def volume():
    return int(pd["speakerVolume"]) * 0.02
